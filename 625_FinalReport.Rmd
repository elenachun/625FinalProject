...
children in the United States${_1}$. There are a variety of factors that can
...
adults and children in the United States"${_2}$. This survey evaluates all
...
and reported solely as birth weight in pounds (lb). Lastly, days of physical activity was binned into categories of 0 days, 1-3 days, 4-6 days, and 7 days as we did not expect big differences in BMI percentiles for each day of physical activity.
...
To explore potential relationships between BMI percentile and various covariates, we conducted exploratory analysis to solidify which variables would be included in our model. Previous literature has suggested that there is a positive correlation
between BMI percentile and birth weight. In the initial plot of BMI percentile and birthweight (Figure 1A), it is unclear whether there is a relationship between the two variables due to the dense data. After taking a random sample of 50 observations from the dataset (Figure 1B), we can see a slight
positive correlation between BMI percentile and birth weight. Additionally, other notable covariate relationships with BMI percentile include race, household food security, and activity. In Figure 1C, differences in BMI percentiles between race are shown and Non-Hispanic Asian children have noticeably lower average BMI percentiles than other races. Differences in BMI percentiles and household food security are shown in Figure 1D and BMI percentiles appear to increase as household food security decreases. A similar relationship was found between BMI percentile and days of physical activity where BMI percentiles decreased as days of physical activity increased. Children with mothers who smoked during pregnancy had higher BMI percentiles than those who did not. Other covariates that did not show discernible differences in BMI percentile were gender and number of meals from fast food place. However, these covariates were included in the analysis as we expect these variables to influence BMI percentile based on previous literature.
...
First, we will examine diagnostics of the linear regression model.The residuals vs. fitted values plot has a random scatter, indicating that the linearity assumption is held. The normal Q-Q plot follows the
linear line relatively well, showing that the residuals are
...
The scale-location plot has
relatively equally spread points across the line, suggesting that the
assumption of constant variance is reasonable. This shows that the
linear regression model is a good fit for the data.
...
1.  Childhood Overweight & Obesity . Published 2023. Accessed November
    14, 2023.\
    [[https://www.cdc.gov/obesity/childhood/index.html\\\\](https://www.cdc.gov/obesity/childhood/index.html\\){.uri}]([https://www.cdc.gov/obesity/childhood/index.html\\](https://www.cdc.gov/obesity/childhood/index.html\){.uri}){.uri}
2.  NHANES - About the National Health and Nutrition Examination Survey.
    Published 2023. Accessed November 14, 2023.\
    [[https://www.cdc.gov/nchs/nhanes/about_nhanes.htm\\\\](https://www.cdc.gov/nchs/nhanes/about_nhanes.htm\\){.uri}]([https://www.cdc.gov/nchs/nhanes/about_nhanes.htm\\](https://www.cdc.gov/nchs/nhanes/about_nhanes.htm\){.uri}){.uri}
