...
Additionally, we combined birth weight in pounds (lbs) and birth weight in ounces (oz) to a single birthweight variable in pounds for easier interpretation. Something to note in this
...
After fitting this model, we were interested in whether the relationship between BMI percentile and a predictor depends on levels of another predictor. To find these interactions, we plotted boxplots of these relationships. For example, this boxplot of food security against BMI percentile filled in by sex shows that the association between BMI percentile and food security is not constant across males and females.
...
Thus, we included the interaction between food security and sex into our regression model:\
...
However, the resulting normal qq plot deviated from a linear line, meaning the normality assumption of residuals was not fulfilled. Thus, we added in a square root transformation to the BMI percentile response variable. Thus the resulting linear regression model is:\
...
Link to GitHub repository: https://github.com/elenachun/625FinalProject
